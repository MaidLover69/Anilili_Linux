...
use crate::models::{AiringEntry, HistoryEntry, NotificationPreference};
...
pub fn get_app_dir() -> Result<PathBuf, AppError> {
    let home = dirs_home().ok_or_else(|| AppError::Other("Could not find home directory".to_string()))?;
...
pub async fn init_db() -> Result<SqlitePool, AppError> {
    ...
    sqlx::query(
        ...
        CREATE TABLE IF NOT EXISTS watch_history (
            ...
            cover           TEXT,
            PRIMARY KEY (anilist_id, episode_number, category)
        );
    ...
    // Ensure columns exist on watch_history if pre-existing from legacy schemas
    ...
    // If legacy 'episode' column exists, drop table or alter
...
pub async fn update_watch_progress(
    ...
    // Delete existing entry if present to avoid legacy primary key conflicts
    let _ = sqlx::query("DELETE FROM watch_history WHERE anilist_id = ?1 AND ABS(episode_number - ?2) < 0.01 AND category = ?3;")
